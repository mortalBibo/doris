...
TEST_F(AdaptiveBlockSizePredictorTest, PredictPerColumnConstraintReducesPredicted) {
    ...
    // Result = min(838860, 209, 65535) = 209.
...
// ── Test: pre-computed per-column hints are used correctly ───────────────────
TEST_F(AdaptiveBlockSizePredictorTest, ConstructorSetsPerColumnHint) {
    // Simulate 2 columns in a 1000-row segment:
    //   col 0: 200000 raw bytes → 200 bytes/row
    //   col 1: 8000000 raw bytes → 8000 bytes/row
    double hint_bpr = (200000.0 + 8000000.0) / 1000.0; // 8200.0
    std::unordered_map<ColumnId, double> col_bpr = {{0, 200.0}, {1, 8000.0}};

    AdaptiveBlockSizePredictor pred(kBlockBytes, kColBytes, hint_bpr, std::move(col_bpr));

    // Verify per-column estimates are set.
    EXPECT_NEAR(pred.col_bytes_per_row_for_test(0), 200.0, 0.01);
    EXPECT_NEAR(pred.col_bytes_per_row_for_test(1), 8000.0, 0.01);

    // Without per-column constraint: predicted = 8MB / 8200.0 = 1023.
    // With per-column constraint (col 1): col_limit = 1MB / 8000.0 = 131.
    // Result = min(1023, 131, 65535) = 131.
    size_t result = pred.predict_next_rows();
...
TEST_F(AdaptiveBlockSizePredictorTest, ConstructorPerColumnHintBlendedByUpdate) {
    // col 0: 400 bytes/row metadata hint (e.g. 400000 raw bytes / 1000 rows)
    double hint_bpr = 400000.0 / 1000.0; // 400.0
    std::unordered_map<ColumnId, double> col_bpr = {{0, 400.0}};
...
TEST_F(AdaptiveBlockSizePredictorTest, PredictUsesCustomProbeRowsWithoutHint) {
    ...
    AdaptiveBlockSizePredictor pred(kBlockBytes, kColBytes, 0.0, {}, custom_probe_rows);
...
TEST_F(AdaptiveBlockSizePredictorTest, PredictUsesCustomProbeRowsWithHint) {
    constexpr size_t custom_probe_rows = 128;
    double hint_bpr = 1.0;
    AdaptiveBlockSizePredictor pred(kBlockBytes, kColBytes, hint_bpr, {}, custom_probe_rows);

    EXPECT_EQ(pred.predict_next_rows(), custom_probe_rows);
}

TEST_F(AdaptiveBlockSizePredictorTest, PredictProbeRowsZeroFallsBackToOne) {
    AdaptiveBlockSizePredictor pred(kBlockBytes, kColBytes, 0.0, {}, 0);
...
TEST_F(AdaptiveBlockSizePredictorTest, PredictProbeRowsOneWorks) {
    AdaptiveBlockSizePredictor pred(kBlockBytes, kColBytes, 0.0, {}, 1);

    EXPECT_EQ(pred.predict_next_rows(), 1u);
}

// ── preferred_block_size_rows tests ─────────────────────────────────────────
...
TEST_F(AdaptiveBlockSizePredictorTest, CustomBlockSizeRows) {
    ...
    AdaptiveBlockSizePredictor pred(kBlockBytes, kColBytes, 0.0, {},
                                    AdaptiveBlockSizePredictor::kDefaultProbeRows, custom_rows);
...
// ── compute_metadata_hints tests ──────────────────────────────────────────────

TEST_F(AdaptiveBlockSizePredictorTest, ComputeMetadataHintsEmptyColumns) {
    auto [hint, col_bpr] =
            AdaptiveBlockSizePredictor::compute_metadata_hints(1000, {});
    EXPECT_DOUBLE_EQ(hint, 0.0);
    EXPECT_TRUE(col_bpr.empty());
}

TEST_F(AdaptiveBlockSizePredictorTest, ComputeMetadataHintsZeroRows) {
    std::vector<AdaptiveBlockSizePredictor::ColumnMetadata> cols = {{0, 10000}};
    auto [hint, col_bpr] = AdaptiveBlockSizePredictor::compute_metadata_hints(0, cols);
    EXPECT_DOUBLE_EQ(hint, 0.0);
    EXPECT_TRUE(col_bpr.empty());
}

TEST_F(AdaptiveBlockSizePredictorTest, ComputeMetadataHintsSingleColumn) {
    // 1000 rows, column 0 has 8000 bytes → 8.0 bytes/row
    std::vector<AdaptiveBlockSizePredictor::ColumnMetadata> cols = {{0, 8000}};
    auto [hint, col_bpr] = AdaptiveBlockSizePredictor::compute_metadata_hints(1000, cols);
    EXPECT_DOUBLE_EQ(hint, 8.0);
    ASSERT_EQ(col_bpr.size(), 1);
    EXPECT_DOUBLE_EQ(col_bpr[0], 8.0);
}

TEST_F(AdaptiveBlockSizePredictorTest, ComputeMetadataHintsMultipleColumns) {
    // 500 rows, col 0 = 2000 bytes (4.0 bpr), col 1 = 3000 bytes (6.0 bpr)
    std::vector<AdaptiveBlockSizePredictor::ColumnMetadata> cols = {{0, 2000}, {1, 3000}};
    auto [hint, col_bpr] = AdaptiveBlockSizePredictor::compute_metadata_hints(500, cols);
    EXPECT_DOUBLE_EQ(hint, 10.0); // (2000 + 3000) / 500
    ASSERT_EQ(col_bpr.size(), 2);
    EXPECT_DOUBLE_EQ(col_bpr[0], 4.0);
    EXPECT_DOUBLE_EQ(col_bpr[1], 6.0);
}

TEST_F(AdaptiveBlockSizePredictorTest, ComputeMetadataHintsSkipsZeroByteColumns) {
    // Column 0 has bytes, column 1 has 0 bytes → excluded from computation
    std::vector<AdaptiveBlockSizePredictor::ColumnMetadata> cols = {{0, 4000}, {1, 0}};
    auto [hint, col_bpr] = AdaptiveBlockSizePredictor::compute_metadata_hints(1000, cols);
    EXPECT_DOUBLE_EQ(hint, 4.0); // only column 0 counted
    ASSERT_EQ(col_bpr.size(), 1);
    EXPECT_DOUBLE_EQ(col_bpr[0], 4.0);
    EXPECT_TRUE(col_bpr.find(1) == col_bpr.end());
}

TEST_F(AdaptiveBlockSizePredictorTest, ComputeMetadataHintsAllZeroBytes) {
    std::vector<AdaptiveBlockSizePredictor::ColumnMetadata> cols = {{0, 0}, {1, 0}};
    auto [hint, col_bpr] = AdaptiveBlockSizePredictor::compute_metadata_hints(1000, cols);
    EXPECT_DOUBLE_EQ(hint, 0.0);
    EXPECT_TRUE(col_bpr.empty());
}
