...
TEST_F(HashJoinProbeOperatorTest, InnerJoinWithAdaptiveBatchSize) {
    ...
    _helper.runtime_state->_query_options.__set_preferred_block_size_bytes(512);
    _helper.runtime_state->_query_options.__set_preferred_block_size_rows(65535);
    ...
    std::vector<Block> build_blocks = {sink_block};
    std::vector<Block> probe_blocks = {probe_block};
    run_test({.join_op_type = TJoinOp::INNER_JOIN},
             {TPrimitiveType::INT, TPrimitiveType::STRING}, {true, false}, {false, true},
             build_blocks, probe_blocks, output_block);

    config::enable_adaptive_batch_size = saved;

    // With adaptive byte budget, result correctness is preserved.
    // Build key (1,null) won't match probe (1,"a"), (2,"b")→(2,"b"), (3,"c")→(3,"c"),
    // (4,"d")→(4,"d"), probe (null,"e") won't match → 3 output rows.
    ASSERT_EQ(output_block.rows(), 3);
    Block sorted_block = sort_block_by_columns(output_block);
